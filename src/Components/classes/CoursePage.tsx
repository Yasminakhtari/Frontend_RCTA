...
//   const user = useSelector((state:any) => state.user);
//   const navigate = useNavigate();
//   const { cart, addToCart, isBooked } = useCart();
//   const sessionRef = useRef<HTMLDivElement | null>(null);
//   const [courses, setCourse] = useState<Course>({} as Course);
//   const [sessions, setSessions] = useState<any[]>([]);
//   const [loading, setLoading] = useState(true);
...
//   // const handlePlayerSelect = (sessionId: number, player: { id: number; name: string }) => {
//   //   // setSelectedPlayers((prev) => {
//   //   //   const prevSelected = prev[sessionId] || [];
//   //   //   if (prevSelected.includes(player)) {
//   //   //     return { ...prev, [sessionId]: prevSelected.filter((p) => p !== player) };
...
//   //   setSelectedPlayers((prev) => ({
//   //     ...prev,
//   //     [sessionId]: prev[sessionId]?.some((p) => p.id === player.id)
//   //       ? prev[sessionId].filter((p) => p.id !== player.id)
//   //       : [...(prev[sessionId] || []), player],
//   //   }));
//   // };
...
//         ? currentPlayers.filter((p) => p.id !== playerId) // Remove player if already selected
//         : [...currentPlayers, { id: playerId, name: playerName }]; // Add player if not selected
...
//       // const totalPrice = selectedSession.price * selectedPlayersCount;
//       // const selectedPlayerNames = selectedPlayers[sessionId] || ["Default Player"]; // Ensure at least 1 player
//       // Ensure selected players are always an array
//     const selectedPlayersList = selectedPlayers[sessionId] || [{ id: "default", name: "Default Player" }];

//     // Extract names for display & store id-name objects for cart
...
//         category: "Sports",
//         image: "/path/to/image",
//         players: selectedPlayerObjects,  // Save selected players
//         groups: `${courses.groups}`
//       };
...
//   function isBooked(sessionId: number): boolean {
//     return cart.some((item: { id: number }) => item.id === sessionId);
//   }
