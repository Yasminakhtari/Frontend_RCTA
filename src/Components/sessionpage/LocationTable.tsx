...
const LocationTable: React.FC = () => {
  ...
  const handleEditSubmit = async () => {
    if (editFormData) {
      try {
        if (isNewLocation) {
          await saveLocation(editFormData);
          ...
          await updateLocation(id, editFormData);
    ...
    <div className="max-w-6xl min-h-screen mx-auto p-4 mt-8">
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between items-center gap-4 mb-4">
        <input
          ...
          value={searchTerm}
          onChange={handleSearch}
          className="w-full sm:w-1/3 px-3 py-2 border rounded focus:outline-none focus:ring-2 focus:ring-blue-500"
        />
        <button
          onClick={handleAddLocation}
        ...
        <table className="min-w-full bg-white text-sm">
          <thead className="bg-blue-600 text-white">
            <tr>
              ...
              <th className="px-4 py-3 text-left hidden sm:table-cell">State</th>
              <th className="px-4 py-3 text-left hidden sm:table-cell">Zip</th>
              ...
              <tr key={index} className="hover:bg-gray-50">
                {editingIndex === index ? (
                  <>
                    ...
                    <td className="flex flex-col space-y-2 justify-center sm:flex-row sm:space-y-0 sm:space-x-2">
                      <button
                        onClick={handleEditSubmit}
                  ...
                  <>
                    ...
                    <td className="hidden md:table-cell">{location.address}</td>
                    <td>{location.city}</td>
                    <td className="hidden sm:table-cell">{location.state}</td>
                    <td className="hidden sm:table-cell">{location.zipCode}</td>
                    <td className="flex flex-col space-y-2 justify-center sm:flex-row sm:space-y-0 sm:space-x-2">
                      <button
                        onClick={() => handleEditClick(index)}
